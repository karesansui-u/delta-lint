...
| 項目 | 値 |
|------|-----|
| 対象 | `/Users/sunagawa/Project/clone-oss` 以下の OSS 12 リポジトリ |
| N | 12 |
| 外部プロキシ | GitHub `open_issues_count`（`gh api repos/{owner}/{repo}` のスナップショット） |
| 取得時点 | 2026-03-28（単一スナップショット、期間フィルタなし） |

サンプル数が小さい前提の **予備的**な帰結であり、論文レベルの一般化ではない。

### Spearman 順位相関

| 指標 | Spearman r | p 値 | 有意（α=0.05） |
|------|------------|------|----------------|
| δ_repo（全体・fallback 含む） | 0.312 | 0.323 | いいえ |
| **δ_cal（`delta_repo_calibrated`）** | **0.653** | **0.021** | **はい** |
| active_count（重みなし件数） | 0.381 | 0.222 | いいえ |

### 読み

- 未校正パターンの **同一 fallback（0.4055 nats）** がリポ間で δ_repo を均質化し、外部プロキシとの順位秩序が消えやすい。
- **I_BASE 実測セルだけの δ_cal** では、プロキシと単調な関係が現れやすく、**校正テーブルが「外の世界」とつながっている兆候**になる。
- 件数だけでは同じ相関が出にくい、というのは **nats で重み付けした和の意味**を支持する。
- ただし N=12・単一プロキシ・単一時点のため、**効果量の推定精度は低く、再現性は未確認**。母集団の偏り（clone-oss に含まれるリポの選定バイアス）にも注意。
