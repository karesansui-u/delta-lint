...
### ミドルウェア・インフラ・バージョンアップ調査の原則

バージョンアップ・ミドルウェア変更・インフラ変更（MySQL, PHP, Redis, WordPress, AWS, nginx 等）に関する調査を行う場合は、以下をすべて実行してから結果を返すこと。指示がなくても自律的に実行すること。

1. **Web調査（必須）**: 該当バージョン間の公式 breaking changes・非互換・既知バグをネットから取得する（公式ドキュメント・リリースノート・トラッカー・コミュニティ情報）
2. **コードベース照合**: 取得した変更点のそれぞれについて、コードベースに影響するパターンを grep/検索で当てる
3. **網羅的な観点で報告**: 「現在問題が起きているもの」だけでなく「将来リスクになりうるもの」も含める
