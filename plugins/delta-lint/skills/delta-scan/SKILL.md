...
4b. User says "リリース後確認", "チェリーピック確認", "チェリーピック漏れ", "リリースミスチェック" → **Release Diff Scan**: ask for release branch names if not provided (e.g. `release/test-2023/KINGSMAN-xxx` and `release/main-2023/KINGSMAN-xxx`), then run Branch Diff Scan (`--scope pr --base origin/<main-branch> --profile strict`) to detect structural contradictions between the two branches. Note: this detects *structural* gaps, not commit-level cherry-pick omissions. For commit-level verification, run `git log` on both branches and compare manually.
