...
2026 年 3 月の短期検証で、Microsoft / Facebook / Bytedance / Sentry / coder / tRPC などの大型 OSS に **PR 16 件（12 リポ）**がマージされました。これは単なる自己評価ではなく、各プロジェクトの maintainer が既存のレビュー基準の上で「修正として妥当」と判断した外部検証です。
