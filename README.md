...
DeltaLint の内部設計（2段階検出パイプライン、3軸スキャンモデル、スコアリング、耐障害性等）の詳細は以下を参照してください。

- **[アーキテクチャ設計書（README）](plugins/delta-lint/README.md)** — データフロー、モジュール構成、スコアリング、設定システム、耐障害性、レビュー観点
- **[ARCHITECTURE.md](plugins/delta-lint/ARCHITECTURE.md)** — モジュールマップ、安定度ガイド、用語集
- **[設計判断記録（ADR）](plugins/delta-lint/docs/decisions/)** — 各設計判断の経緯と根拠
